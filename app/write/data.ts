/**
 * =============================================================================
 *  The Daily Count
 * =============================================================================
 *
 *  Description: Write Page, Data
 *
 *  Created by:   David Strickland
 *  Company:      DCSS Web Development LLC
 *  Created on:   2026-07-18
 * =============================================================================
 */

export const PAGE_EYEBROW = "Owner only";

export const PAGE_HEADING = "Write a new entry";

export const PAGE_INTRO =
  "Not linked anywhere on the site, this is where new journal entries get published.";

export const PAGE_WATERMARK = "WRITE";

export const PAGE_BLURB = "Publish a new journal entry.";

export const MANAGE_ENTRIES_HEADING = "Existing entries";

export const MANAGE_ENTRIES_EMPTY_TEXT = "No entries yet.";

export const DELETE_CONFIRM_TEXT =
  "Delete this entry and all of its comments? This can't be undone.";

export const PENDING_COMMENTS_HEADING = "Pending comments";

export const PENDING_COMMENTS_EMPTY_TEXT = "No comments waiting for review.";

export const REJECT_COMMENT_CONFIRM_TEXT =
  "Reject and delete this comment? This can't be undone.";
